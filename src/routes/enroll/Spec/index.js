...
@connect(({ detail, enroll }) => ({ detail, enroll }))
export default class Spec extends Component {
  ...
  componentWillMount() {
    // console.log(props);
    const { enroll: { current } } = this.props;
  ...
  // 确认订单
  goConfirm = () => {
    const { dispatch, enroll: { current } } = this.props;
    const skuKeyList = [];
    let spec = {};
    ...
    // };
    // const url = this.state.categoryType === '1' ? 'detail/fetchConfirmSubmitError' :
    // 'detail/fetchNormalConfirmSubmitError';
    ...
    dispatch({
      ...
      dispatch(routerRedux.push(
        `/enroll/enrollPage?skuId=${this.state.skuId}&quantity=${this.state.quantity}`
      ));
      //   });
      // });
    });
  ...
  handleSpecItemClick = (e) => {
    ...
    const { enroll: { current: { skuList } } } = this.props;
    // console.log(e);
