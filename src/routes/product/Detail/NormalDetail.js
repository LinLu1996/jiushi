...
export default class NormalDetail extends Component {
  constructor(props) {
    super(props);
    this.param = { ...urlParse(props.location.search), categoryType: '2' };
    this.state = {
      title: '商品详情',
      introduction: {
        __html: '订单详情',
  ...
  /** ****************渲染价格时间************************ */
  renderMoney = () => {
    const { validStartDateStr, validEndDateStr } = this.state;
    return (
      <div>
        <p className={styles.time}>
          时间：{validStartDateStr && moment(validStartDateStr).format('YYYY-MM-DD')} ~ {validEndDateStr && moment(validEndDateStr).format('YYYY-MM-DD')}
        </p>
  ...
  renderEventInfo = () => {
    ...
    return (
      <div>
        ...
        {current && !current.validStatus && categoryType === '2' && this.renderTime()}
        {current && current.validStatus && categoryType === '2' && this.renderMoney()}
