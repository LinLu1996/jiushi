import { PullToRefresh, Carousel, ActivityIndicator } from 'antd-mobile';
...
@connect(({ product, advertisment, loading }) => (
  {
    product,
    advertisment,
...
export default class Product extends Component {
  ...
  loadData = () => {
    const { dispatch, product: { page, hasMore } } = this.props;
    if (!hasMore) {
      return false;
    }
    dispatch({
  ...
  renderProductList = () => {
    const { product: { list, loadFlag }, dispatch } = this.props;
    if (loadFlag) {
      return <Loading />;
    }
  ...
  render() {
    const { advertisment: { banner: ads }, product: { hasMore } } = this.props;
    ...
    return (
      <FullScreenFlex style={{ flexDirection: 'column', overflow: 'scroll' }} className="home">
        <PullToRefresh
          ...
          indicator={{ activate: '松手更新', deactivate: '上拉刷新', finish: hasMore ? '' : '没有更多商品' }}
          direction="up"
